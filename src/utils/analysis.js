...
export function analyzeResume(text = '') {
  ...
  const cleanedText = normalizedText.toLowerCase()
  ...
  const domainFromKeywords = Object.entries(DOMAIN_KEYWORDS).find(([domain, words]) =>
    words.some(word => cleanedText.includes(word.toLowerCase()))
  )?.[0]
  ...
  // Calculate ATS score
  let score = 30
  score += Math.min(35, skills.length * 3) // skills bonus

  const domainScore = domain && domain !== 'general' ? 10 : 0
  score += domainScore

  const hasExperienceSection = /experience|employment|work|professional/i.test(cleanedText)
  if (hasExperienceSection) score += 10

  const hasEducation = /education|degree|university|college|bachelor|master/i.test(cleanedText)
  if (hasEducation) score += 8

  const hasProjects = /project|built|developed|created|implemented/i.test(cleanedText)
  if (hasProjects) score += 7

  const hasMetrics = /[0-9]+%|increased|improved|reduced|grew|scaled/i.test(cleanedText)
  if (hasMetrics) score += 5

  // Add deterministic content depth score (0-5)
  score += Math.min(5, Math.floor(cleanedText.length / 400))
  ...
  if (!cleanedText.includes('summary') && !cleanedText.includes('objective')) {
  ...
  return { score, skills, missing, suggestions, domain }
}

export function matchJob(resumeText = '', jobText = '') {
  const normalizedResume = typeof resumeText === 'string' ? resumeText : String(resumeText || '')
  const normalizedJob = typeof jobText === 'string' ? jobText : String(jobText || '')

  if (!normalizedJob.trim()) return { percent: 0, matchedSkills: [], missingSkills: [], suggestions: [] }
...
export function generateSkillRoadmap(missingSkills) {
